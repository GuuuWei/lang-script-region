...
Typr["parse"] = function (buff) {
  ...
  // Detect if the format is WOFF2
  if (tag == "wOF2") {
    var decompressedData = Typr._decompressWOFF2(data);
    return Typr._parseFontData(decompressedData);
  } else if (tag == "ttcf") {
    return Typr._parseTTC(data);
  } else {
    return [Typr._parseFontData(data, 0)];
  }
};

// Decompress WOFF2 data using unbrotli
Typr._decompressWOFF2 = function (data) {
  const brotliDecode = globalThis.unbrotli;
  var bin = Typr["B"];

  // WOFF2 header parsing
  var woff2HeaderSize = 48; // WOFF2 header is 48 bytes
  var compressedDataOffset = woff2HeaderSize;
  let decompressedData;

  // Extracting compressed font data
  var compressedDataLength = bin.readUint(data, 44); // Offset 44 contains the length of compressed data
  var compressedData = data.slice(
    compressedDataOffset,
	compressedDataOffset + compressedDataLength
  );
  console.log(compressedData);
  // Using unbrotli.js to decompress the font data
  if (brotliDecode) {
    decompressedData = brotliDecode(new Uint8Array(compressedData));
...
// Parse TTC format fonts (for collections)
Typr._parseTTC = function (data) {
  var bin = Typr["B"];
  var offset = 4;
  ...
  for (var i = 0; i < numF; i++) {
    ...
    offset += 4;
    fonts.push(Typr._parseFontData(data, foff));
  }
  return fonts;
};

// Parse standard font data (TrueType or OpenType)
Typr._parseFontData = function (data, offset) {
  ...
  var tmap = {};
  return readFont(data, 0, offset, tmap);
};
...
// Assuming unbrotli is available as a utility function for decompression
