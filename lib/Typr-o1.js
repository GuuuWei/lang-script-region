...
// Include the WOFF2 decoder library
// You need to include woff2.js in your project for this to work.
// You can find it here: https://github.com/fontello/woff2.js
// Make sure to include it before this script or adjust the code accordingly.
...
Typr["parse"] = function (buff) {
  ...
  if (tag == "ttcf") {
    // Handle TrueType Collection Font
    var offset = 4;
    ...
    // Handle WOFF Font
    var decompressedData = Typr["WOFF"].decompress(buff);
    data = new Uint8Array(decompressedData);
    return [readFont(data, 0, 0, tmap)];
  } else if (tag == "wOF2") {
    // Handle WOFF2 Font
    var decompressedData = Typr["WOFF2"].decompress(buff);
    data = new Uint8Array(decompressedData);
    return [readFont(data, 0, 0, tmap)];
  } else {
    // Handle standard font
    return [readFont(data, 0, 0, tmap)];
  }
};

// WOFF Decompression (for wOFF tag)
Typr["WOFF"] = {
  decompress: function (buff) {
    var bin = Typr["B"];
    var data = new Uint8Array(buff);
    var reader = {
      offset: 0,
      data: data,
      readUint32: function () {
        var val = bin.readUint(this.data, this.offset);
        this.offset += 4;
        return val;
      },
      readUint16: function () {
        var val = bin.readUshort(this.data, this.offset);
        this.offset += 2;
        return val;
      },
      readBytes: function (length) {
        var bytes = this.data.slice(this.offset, this.offset + length);
        this.offset += length;
        return bytes;
      },
    ...
    // Read WOFF Header
    var signature = bin.readASCII(data, 0, 4);
    if (signature != "wOFF") {
      throw "Invalid WOFF font!";
    }
    reader.offset = 4; // Skip signature

    var flavor = reader.readUint32(); // Original font signature
    var length = reader.readUint32(); // Total size of the WOFF file
    var numTables = reader.readUint16();
    reader.offset += 2; // Skip reserved
    var totalSfntSize = reader.readUint32();
    reader.offset += 4 * 2; // Skip majorVersion, minorVersion
    reader.offset += 4 * 2; // Skip metaOffset, metaLength, metaOrigLength
    reader.offset += 4 * 3; // Skip privOffset, privLength

    // Read Table Directory
    var tableEntries = [];
    for (var i = 0; i < numTables; i++) {
      var tag = bin.readASCII(data, reader.offset, 4);
      reader.offset += 4;
    ...
    // Reconstruct the original font data
    var outBuffer = new Uint8Array(totalSfntSize);
    var outOffset = 0;

    // Write SFNT header
    bin.writeASCII(outBuffer, outOffset, bin.readASCII(data, 4, 4)); // Flavor
    outOffset += 4;
    bin.writeUshort(outBuffer, outOffset, numTables); // numTables
    outOffset += 2;

    // Calculate searchRange, entrySelector, rangeShift
    var maxPower2 = 1;
    while (maxPower2 * 2 <= numTables) maxPower2 *= 2;
    var searchRange = maxPower2 * 16;
    var entrySelector = Math.log(maxPower2) / Math.log(2);
    var rangeShift = numTables * 16 - searchRange;

    bin.writeUshort(outBuffer, outOffset, searchRange);
    outOffset += 2;
    bin.writeUshort(outBuffer, outOffset, entrySelector);
    outOffset += 2;
    bin.writeUshort(outBuffer, outOffset, rangeShift);
    outOffset += 2;

    // Prepare for table records
    var tableRecordOffset = outOffset;
    outOffset += numTables * 16;

    // Decompress and write table data
    for (var i = 0; i < numTables; i++) {
      var entry = tableEntries[i];
      var compData = data.slice(entry.offset, entry.offset + entry.compLength);
      var origData;

      if (entry.compLength != entry.origLength) {
        // Decompress using zlib
        origData = pako["inflate"](compData);
      } else {
        // No compression
        origData = compData;
      }
      ...
      // Write table record
      bin.writeASCII(outBuffer, tableRecordOffset, entry.tag);
      tableRecordOffset += 4;
      var checksum = Typr["WOFF"].calcChecksum(origData);
      bin.writeUint(outBuffer, tableRecordOffset, checksum);
      tableRecordOffset += 4;
      bin.writeUint(outBuffer, tableRecordOffset, outOffset);
      tableRecordOffset += 4;
      bin.writeUint(outBuffer, tableRecordOffset, entry.origLength);
      tableRecordOffset += 4;

      // Write table data
      outBuffer.set(origData, outOffset);
      outOffset += entry.origLength;

      // Align to 4 bytes
      while (outOffset % 4 != 0) outOffset++;
    }
  ...
  calcChecksum: function (data) {
    var bin = Typr["B"];
    var sum = 0;
    var nLongs = Math.floor((data.length + 3) / 4);
    for (var i = 0; i < nLongs; i++) {
      var value = bin.readUint(data, i * 4);
      sum = (sum + value) >>> 0;
    }
    return sum;
  },
...
// WOFF2 Decompression (for wOF2 tag)
Typr["WOFF2"] = {
  decompress: function (buff) {
    // You need to include woff2.js in your project for this to work.
    // https://github.com/fontello/woff2.js
    // Make sure to include the script before this code.

    var outBuffer = window["woff2"].decompress(buff);
    return outBuffer;
  },
};

// Other existing code remains the same...
// Typr["findTable"], Typr["B"], Typr["T"], etc.

// Note: Ensure that you have included the necessary libraries for Brotli and WOFF2 decompression.
// For WOFF2, you can use the woff2.js library: https://github.com/fontello/woff2.js
// For Brotli decoding, woff2.js includes a Brotli decoder.

// Also, include pako.js for zlib decompression used in WOFF fonts if not already included.
// You can find it here: https://github.com/nodeca/pako

// Usage example:
// var fontArrayBuffer = ...; // Load your font data as ArrayBuffer
// var font = Typr.parse(fontArrayBuffer);
